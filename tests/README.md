...
The focused regression executables use a small first-party harness. Forty-four focused suites
...
| `Lorenzo2DRenderStatisticsTests` | Saturating ordered draw telemetry, material/shader switches, sprite/batch/surface/post-process accounting, pipeline/composition propagation, tilemap culling bridge, and diagnostic-counter export | `xvfb` | 90 s |
...
`Lorenzo2DSpriteBatchTests`, `Lorenzo2DRenderCompositionExecutionTests`,
`Lorenzo2DRenderStatisticsTests`, and `Lorenzo2DRendererTests` use the Xvfb partition because their
current execution path may construct or exercise SFML graphics resources. The remaining suites are
executed with display variables removed.
