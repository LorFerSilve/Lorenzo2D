...
namespace l2d::detail
{
                                   ...
                                   std::vector<PhysicsContactEvent2D>& events)
    {
        ...
        while (previousIndex < previousContacts.size() || currentIndex < currentContacts.size())
        {
            if (previousIndex >= previousContacts.size())
            {
                events.push_back({PhysicsContactPhase2D::Begin, currentContacts[currentIndex]});
                ++currentIndex;
                continue;
            }
            ...
            {
                events.push_back({PhysicsContactPhase2D::End, previousContacts[previousIndex]});
                ++previousIndex;
                continue;
            }

            const PhysicsContact2D& previous = previousContacts[previousIndex];
            const PhysicsContact2D& current = currentContacts[currentIndex];

            if (samePhysicsContactKey(previous, current))
            {
                events.push_back({PhysicsContactPhase2D::Stay, current});
                ++previousIndex;
                ++currentIndex;
            }
            else if (physicsContactLess(previous, current))
            {
                events.push_back({PhysicsContactPhase2D::End, previous});
                ++previousIndex;
            }
            else
            {
                events.push_back({PhysicsContactPhase2D::Begin, current});
                ++currentIndex;
            }
        }
    }
