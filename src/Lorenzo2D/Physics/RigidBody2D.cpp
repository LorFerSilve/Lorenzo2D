...
namespace l2d
{
    namespace
    {
        ...
        {
            if (!isFinite(value)) return {0.f, 0.f};
        ...
        {
            if (!std::isfinite(mass) || mass < MIN_MASS) return MIN_MASS;
        ...
        void moveSafely(GameObject& gameObject, sf::Vector2f direction, double scale)
        {
            ...
            const double x =
                static_cast<double>(position.x) + static_cast<double>(direction.x) * scale;
            const double y =
                static_cast<double>(position.y) + static_cast<double>(direction.y) * scale;
            ...
            gameObject.transform.setPosition({static_cast<float>(x), static_cast<float>(y)});
        }
    }

    RigidBody2D::RigidBody2D()
        : m_bodyType(BodyType2D::Dynamic), m_velocity(0.f, 0.f), m_acceleration(0.f, 0.f),
          m_forceAccumulator(0.f, 0.f), m_mass(1.f), m_useGravity(false), m_gravityScale(1.f),
          m_isGrounded(false)
    {
    }
    ...
    void RigidBody2D::setBodyType(BodyType2D bodyType)
    {
        ...
        if (m_bodyType == BodyType2D::Static) m_velocity = {0.f, 0.f};
    }
    ...
    void RigidBody2D::addVelocity(sf::Vector2f velocity)
    {
        ...
        if (isFinite(updatedVelocity)) m_velocity = updatedVelocity;
    }
    ...
    void RigidBody2D::addForce(sf::Vector2f force)
    {
        ...
        if (isFinite(updatedForce)) m_forceAccumulator = updatedForce;
    }

    void RigidBody2D::applyImpulse(sf::Vector2f impulse)
    {
        if (m_bodyType != BodyType2D::Dynamic || !isFinite(impulse)) return;

        const double inverseBodyMass = 1.0 / static_cast<double>(m_mass);
        const double x =
            static_cast<double>(m_velocity.x) + static_cast<double>(impulse.x) * inverseBodyMass;
        const double y =
            static_cast<double>(m_velocity.y) + static_cast<double>(impulse.y) * inverseBodyMass;
        const double maximum = static_cast<double>(std::numeric_limits<float>::max());
        ...
        m_velocity = {static_cast<float>(x), static_cast<float>(y)};
    }
    ...
    {
        if (m_bodyType != BodyType2D::Dynamic) return 0.f;
    ...
    {
        if (!std::isfinite(gravityScale)) gravityScale = DEFAULT_GRAVITY_SCALE;
    ...
    void RigidBody2D::integrate(float deltaTime, sf::Vector2f worldGravity)
    {
        ...
        else if (m_bodyType == BodyType2D::Kinematic)
        {
            ...
            {
                moveSafely(*gameObject, m_velocity, static_cast<double>(deltaTime));
            }
        }
        else
        {
            ...
            const double nextVelocityX =
                static_cast<double>(m_velocity.x) + accelerationX * static_cast<double>(deltaTime);
            const double nextVelocityY =
                static_cast<double>(m_velocity.y) + accelerationY * static_cast<double>(deltaTime);
            const double maximum = static_cast<double>(std::numeric_limits<float>::max());

            if (std::isfinite(nextVelocityX) && std::isfinite(nextVelocityY) &&
                std::fabs(nextVelocityX) <= maximum && std::fabs(nextVelocityY) <= maximum)
            {
                m_velocity = {static_cast<float>(nextVelocityX), static_cast<float>(nextVelocityY)};
            }
