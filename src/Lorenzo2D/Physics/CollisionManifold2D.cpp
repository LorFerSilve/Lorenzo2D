...
namespace l2d
{
    namespace
    {
        ...
        bool circleCircleManifold(
            const CircleCollider2D& circleA,
        ...
        )
        {
            ...
            if (!isFinite(centerA) || !isFinite(centerB))
                return false;

            const double deltaX =
                static_cast<double>(centerB.x) - centerA.x;
            const double deltaY =
                static_cast<double>(centerB.y) - centerA.y;
            ...
            const double pointOnA_X =
                static_cast<double>(centerA.x) + normal.x * radiusA;
            const double pointOnA_Y =
                static_cast<double>(centerA.y) + normal.y * radiusA;
            const double pointOnB_X =
                static_cast<double>(centerB.x) - normal.x * radiusB;
            const double pointOnB_Y =
                static_cast<double>(centerB.y) - normal.y * radiusB;

            return setManifold(
                normal,
                finiteVector(
                    pointOnA_X + (pointOnB_X - pointOnA_X) * 0.5,
        ...
        )
        {
            ...
            const double overlapX = std::min(
                static_cast<double>(maximumA.x),
                static_cast<double>(maximumB.x)
            ) - std::max(
                static_cast<double>(minimumA.x),
                static_cast<double>(minimumB.x)
            );
            const double overlapY = std::min(
                static_cast<double>(maximumA.y),
                static_cast<double>(maximumB.y)
            ) - std::max(
                static_cast<double>(minimumA.y),
                static_cast<double>(minimumB.y)
            );
            ...
            double penetration =
                static_cast<double>(maximumA.x) - minimumB.x;
            sf::Vector2f normal = { 1.f, 0.f };

            const double moveFirstRight =
                static_cast<double>(maximumB.x) - minimumA.x;
            ...
            const double moveFirstUp =
                static_cast<double>(maximumA.y) - minimumB.y;
            ...
            const double moveFirstDown =
                static_cast<double>(maximumB.y) - minimumA.y;
            ...
            const double overlapMinimumX = std::max(
                static_cast<double>(minimumA.x),
                static_cast<double>(minimumB.x)
            );
            const double overlapMaximumX = std::min(
                static_cast<double>(maximumA.x),
                static_cast<double>(maximumB.x)
            );
            const double overlapMinimumY = std::max(
                static_cast<double>(minimumA.y),
                static_cast<double>(minimumB.y)
            );
            const double overlapMaximumY = std::min(
                static_cast<double>(maximumA.y),
                static_cast<double>(maximumB.y)
            );
            ...
            return setManifold(
                ...
                finiteVector(
                    overlapMinimumX +
                        (overlapMaximumX - overlapMinimumX) * 0.5,
                    overlapMinimumY +
                        (overlapMaximumY - overlapMinimumY) * 0.5
                ),
                penetration,
                manifold
            );
        ...
        bool circleBoxManifold(
            const CircleCollider2D& circle,
            const BoxCollider2D& box,
            CollisionManifold2D& manifold
        )
        {
            ...
            if (distance > 0.0)
            {
                return setManifold(
                    ...
                    finiteVector(closestX, closestY),
                    radius - distance,
                    manifold
                );
            }
    ...
    )
    {
        ...
        {
            const auto* circleA =
                dynamic_cast<const CircleCollider2D*>(&colliderA);
            const auto* circleB =
                dynamic_cast<const CircleCollider2D*>(&colliderB);
        ...
        if (
            colliderA.type() == ColliderType::Box &&
            colliderB.type() == ColliderType::Circle
        )
        {
        ...
        )
        {
            ...
            return boxA != nullptr && boxB != nullptr &&
                boxBoxManifold(*boxA, *boxB, manifold);
        }
