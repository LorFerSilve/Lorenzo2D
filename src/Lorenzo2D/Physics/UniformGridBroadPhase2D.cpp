    ...
    namespace detail
    {
        namespace
        {
            ...
            struct GridCellHash2D
            {
                ...
                {
                    const std::size_t xHash = std::hash<std::int64_t>{}(cell.x);
                    const std::size_t yHash = std::hash<std::int64_t>{}(cell.y);

                    return xHash ^ (yHash << 1u);
                }
            };

            bool pairLess(const BroadPhasePair2D& left, const BroadPhasePair2D& right)
            {
            ...
            bool samePair(const BroadPhasePair2D& left, const BroadPhasePair2D& right)
            {
                return left.first == right.first && left.second == right.second;
            }

            bool isEligiblePair(const std::vector<BroadPhaseProxy2D>& proxies, std::size_t first,
                                std::size_t second)
            {
                return first != second && (proxies[first].moving || proxies[second].moving);
            }

            bool boundsOverlap(const BroadPhaseProxy2D& first, const BroadPhaseProxy2D& second)
            {
                if (!first.boundsValid || !second.boundsValid) return true;
            ...
            void appendPair(const std::vector<BroadPhaseProxy2D>& proxies, std::size_t first,
                            std::size_t second, std::vector<BroadPhasePair2D>& pairs)
            {
                if (!isEligiblePair(proxies, first, second)) return;
            ...
            void appendSpatialPair(const std::vector<BroadPhaseProxy2D>& proxies, std::size_t first,
                                   std::size_t second, std::vector<BroadPhasePair2D>& pairs)
            {
                if (first == second || !boundsOverlap(proxies[first], proxies[second]))
                {
                    return;
                }
            ...
            void sortAndDeduplicate(std::vector<BroadPhasePair2D>& pairs)
            {
                ...
                pairs.erase(std::unique(pairs.begin(), pairs.end(), samePair), pairs.end());
            }

            bool toCellCoordinate(float coordinate, float cellSize, std::int64_t& result)
            {
                if (!std::isfinite(coordinate)) return false;

                const long double cell = std::floor(static_cast<long double>(coordinate) /
                                                    static_cast<long double>(cellSize));
                const long double coordinateLimit = std::ldexp(1.0L, 63);

                if (!std::isfinite(cell) || cell < -coordinateLimit || cell >= coordinateLimit)
                {
                    return false;
                }
            ...
            bool inclusiveSpan(std::int64_t minimum, std::int64_t maximum, std::uint64_t& result)
            {
                if (maximum < minimum) return false;

                const std::uint64_t difference =
                    static_cast<std::uint64_t>(maximum) - static_cast<std::uint64_t>(minimum);

                if (difference == std::numeric_limits<std::uint64_t>::max()) return false;

                result = difference + 1u;
                return true;
            }

            bool gridRange(const BroadPhaseProxy2D& proxy, float cellSize,
                           std::uint32_t maxCellsPerProxy, GridCell2D& minimum, GridCell2D& maximum)
            {
                if (!proxy.boundsValid) return false;

                if (proxy.minimum.x > proxy.maximum.x || proxy.minimum.y > proxy.maximum.y)
                {
                    return false;
                }

                if (!toCellCoordinate(proxy.minimum.x, cellSize, minimum.x) ||
                ...
                if (!inclusiveSpan(minimum.x, maximum.x, columns) ||
                ...
                return columns <= maximumCells && rows <= maximumCells / columns;
            }
            ...
            std::size_t multiplySaturated(std::size_t left, std::size_t right)
            {
                const std::size_t maximum = std::numeric_limits<std::size_t>::max();

                if (left != 0 && right > maximum / left) return maximum;
            ...
            std::size_t pairCombinationCount(std::size_t count)
            {
                ...
                if (count % 2 == 0) return multiplySaturated(count / 2, count - 1);
        ...
        std::size_t countBruteForcePairs(const std::vector<BroadPhaseProxy2D>& proxies)
        {
            ...
            {
                if (proxy.moving) ++movingCount;
            }

            const std::size_t staticCount = proxies.size() - movingCount;

            return addSaturated(pairCombinationCount(movingCount),
        ...
        BroadPhaseBuildResult2D buildBruteForcePairs(const std::vector<BroadPhaseProxy2D>& proxies)
        {
            ...
            for (std::size_t first = 0; first < movingProxies.size(); ++first)
            {
                for (std::size_t second = first + 1; second < movingProxies.size(); ++second)
                {
                    appendPair(proxies, movingProxies[first], movingProxies[second], result.pairs);
                }

                for (const std::size_t staticProxy : staticProxies)
                {
                    appendPair(proxies, movingProxies[first], staticProxy, result.pairs);
                }
            }
        ...
        {
            if (!std::isfinite(cellSize) || cellSize <= 0.f || maxCellsPerProxy == 0)
            {
                BroadPhaseBuildResult2D result = buildBruteForcePairs(proxies);
                result.fallbackProxyCount = proxies.size();
                return result;
            }

            using CellMap =
                std::unordered_map<GridCell2D, std::vector<std::size_t>, GridCellHash2D>;
            ...
            for (std::size_t index = 0; index < proxies.size(); ++index)
            {
                if (proxies[index].moving) movingProxies.push_back(index);

                GridCell2D minimum;
                GridCell2D maximum;

                if (!gridRange(proxies[index], cellSize, maxCellsPerProxy, minimum, maximum))
                {
            ...
            for (std::size_t index = 0; index < proxies.size(); ++index)
            {
                if (!proxies[index].moving || !inserted[index]) continue;
                ...
                for (std::int64_t y = minimum.y;; ++y)
                {
                    for (std::int64_t x = minimum.x;; ++x)
                    {
                        ...
                        if (cell != cells.end())
                        {
                            ...
                            {
                                appendSpatialPair(proxies, index, occupant, result.pairs);
                            }
                        }
            ...
            for (const std::size_t fallback : fallbackProxies)
            {
                ...
                {
                    for (std::size_t other = 0; other < proxies.size(); ++other)
                    {
                        appendSpatialPair(proxies, fallback, other, result.pairs);
                    }
                }
                else
                {
                    ...
                    {
                        appendSpatialPair(proxies, fallback, moving, result.pairs);
                    }
                }
            }
