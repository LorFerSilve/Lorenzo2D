...
namespace l2d::renderer_detail
{
    inline float maximumSafeViewExtent()
    {
        static const float value =
            std::sqrt(std::numeric_limits<float>::max()) / 4.f;
    ...
    inline bool isSafeCameraCoordinate(float value)
    {
        return std::isfinite(value) &&
            std::abs(value) <= maximumSafeViewExtent();
    }

    inline bool isSafeCameraPosition(sf::Vector2f value)
    {
        return isSafeCameraCoordinate(value.x) &&
            isSafeCameraCoordinate(value.y);
    }
    ...
    inline bool hasSafeAxisAlignedBounds(
    ...
    {
        if (
            !isSafeDrawablePosition(position) ||
            !isFinite(size) ||
            size.x < 0.f ||
            size.y < 0.f ||
            !std::isfinite(padding) ||
            padding < 0.f ||
            !isSafeDrawableCoordinate(size.x) ||
            !isSafeDrawableCoordinate(size.y) ||
            !isSafeDrawableCoordinate(padding)
        )
        {
            return false;
        }
        ...
        const double minimumX =
            static_cast<double>(position.x) - widenedPadding;
        const double minimumY =
            static_cast<double>(position.y) - widenedPadding;
        const double maximumX =
            static_cast<double>(position.x) +
            static_cast<double>(size.x) + widenedPadding;
        const double maximumY =
            static_cast<double>(position.y) +
            static_cast<double>(size.y) + widenedPadding;

        return isSafeDrawableCoordinate(minimumX) &&
            isSafeDrawableCoordinate(minimumY) &&
            isSafeDrawableCoordinate(maximumX) &&
            isSafeDrawableCoordinate(maximumY);
    }
    ...
    inline bool hasSafeTransformedBounds(
        const sf::FloatRect& localBounds,
        const TransformState& state
    )
    {
        if (!isFinite(state) || !isSafeDrawablePosition(state.position))
            return false;

        const double left = static_cast<double>(localBounds.position.x);
        const double top = static_cast<double>(localBounds.position.y);
        const double right =
            left + static_cast<double>(localBounds.size.x);
        const double bottom =
            top + static_cast<double>(localBounds.size.y);
        ...
        const double angle =
            static_cast<double>(normalizedRotationDegrees(state.rotation)) *
            degreesToRadians;
        ...
        for (const double x : xCoordinates)
        {
            for (const double y : yCoordinates)
            {
                const double scaledX =
                    x * static_cast<double>(state.scale.x);
                const double scaledY =
                    y * static_cast<double>(state.scale.y);

                if (
                    !isSafeDrawableCoordinate(scaledX) ||
                    !isSafeDrawableCoordinate(scaledY)
                )
                {
                    return false;
                }
                ...
                if (
                    !isSafeDrawableCoordinate(transformedX) ||
