...
namespace l2d
{
    namespace
    {
        ...
        {
            const float callbackFixedDeltaTime = static_cast<float>(config.fixedDeltaTime);

            if (!std::isfinite(config.fixedDeltaTime) || config.fixedDeltaTime <= 0.0 ||
                !std::isfinite(callbackFixedDeltaTime) || callbackFixedDeltaTime <= 0.f)
            {
                config.fixedDeltaTime = DEFAULT_FIXED_DELTA_TIME;
            }

            if (!std::isfinite(config.maximumFrameDeltaTime) || config.maximumFrameDeltaTime <= 0.0)
            {
                config.maximumFrameDeltaTime = DEFAULT_MAXIMUM_FRAME_DELTA_TIME;
            }

            if (config.maximumTicksPerFrame == 0)
            {
                config.maximumTicksPerFrame = DEFAULT_MAXIMUM_TICKS_PER_FRAME;
            }
        ...
        {
            if (!std::isfinite(tickCount)) return std::numeric_limits<std::uint64_t>::max();

            if (tickCount <= 0.0) return 0;

            const double maximum = static_cast<double>(std::numeric_limits<std::uint64_t>::max());

            if (tickCount >= maximum) return std::numeric_limits<std::uint64_t>::max();
        ...
        NearbyInteger snapToNearbyInteger(double value, double remainderRatio)
        {
            if (!std::isfinite(value)) return {value, false};
            ...
            const double tolerance = std::min(8.0 * std::numeric_limits<double>::epsilon() *
            ...
            const bool remainderIsAtBoundary =
                ...
                (remainderRatio <= tolerance || 1.0 - remainderRatio <= tolerance);

            if (distance <= tolerance && (distance > 0.0 || remainderIsAtBoundary))
            {
        ...
        double interpolationAlpha(double accumulator, double fixedDeltaTime)
        {
            ...
            if (alpha >= 1.0) return std::nextafter(1.0, 0.0);
    ...
    FixedStepFrame FixedStepScheduler::advance(double rawDeltaTime)
    {
        ...
        frame.frameDeltaTime = std::min(frame.rawDeltaTime, m_config.maximumFrameDeltaTime);
        frame.clampedFrameTime = std::max(0.0, frame.rawDeltaTime - frame.frameDeltaTime);

        m_frameCount = saturatingAdd(m_frameCount, std::uint64_t{1});
        m_realElapsedTime = saturatingAdd(m_realElapsedTime, frame.rawDeltaTime);
        m_clampedFrameTime = saturatingAdd(m_clampedFrameTime, frame.clampedFrameTime);

        m_accumulator = saturatingAdd(m_accumulator, frame.frameDeltaTime);

        double remainder = std::fmod(m_accumulator, m_config.fixedDeltaTime);

        if (!std::isfinite(remainder) || remainder < 0.0) remainder = 0.0;

        const NearbyInteger tickRatio = snapToNearbyInteger(m_accumulator / m_config.fixedDeltaTime,
        ...
        if (tickRatio.value >= 1.0)
        {
            ...
            const double ticksToRun =
                std::min(availableTicks, static_cast<double>(m_config.maximumTicksPerFrame));
            ...
            if (tickRatio.snapped) remainder = 0.0;

            const double wholeStepTime = m_accumulator - remainder;
            const double executedSimulationTime = ticksToRun * m_config.fixedDeltaTime;

            frame.droppedSimulationTime = std::max(0.0, wholeStepTime - executedSimulationTime);

            m_accumulator = remainder;

            m_tickCount = saturatingAdd(m_tickCount, static_cast<std::uint64_t>(frame.ticksToRun));

            m_droppedTickCount = saturatingAdd(m_droppedTickCount, frame.droppedTicks);

            m_simulationTime = saturatingAdd(m_simulationTime, executedSimulationTime);

            m_droppedSimulationTime =
                saturatingAdd(m_droppedSimulationTime, frame.droppedSimulationTime);
        }

        frame.interpolationAlpha = l2d::interpolationAlpha(m_accumulator, m_config.fixedDeltaTime);
